...
def simulate(time_periods, mynetwork, my_tariffs, results, status_callback=None):
    ...
    for p in mynetwork.get_participants():
        ...
        for time in time_periods:
            ...
            if 'TOU' in network_tariff_type:
                variable_tariff = get_tou_variable_tariff(time, my_tariffs.get_duos_on_grid_import_variable(time,network_tariff_type))
                results.set_participant_duos_payments(time, p.get_id(), variable_tariff * external_grid_import)
    ...
    for time in time_periods:
        grid_import_revenue_variable = sum([results.get_participant_duos_payments(time, p.get_id()) for p in mynetwork.get_participants()])
    ...
    for p in mynetwork.get_participants():
        ...
        for time in time_periods:
            ...
            # if network_tariff_type == 'LV TOU <100MWh' or network_tariff_type == 'LV Business TOU_Interval meter' or network_tariff_type == 'Small Business - Opt in Demand':
            if 'TOU' in network_tariff_type:
                variable_tariff = get_tou_variable_tariff(time, my_tariffs.get_tuos_on_grid_import_variable(time,network_tariff_type))
                results.set_participant_tuos_payments(time, p.get_id(), variable_tariff * external_grid_import)
    ...
    for time in time_periods:
        tnsp_grid_import_revenue_variable = sum([results.get_participant_tuos_payments(time, p.get_id()) for p in mynetwork.get_participants()])
    ...
    for p in mynetwork.get_participants():
        ...
        for time in time_periods:
            ...
            # if network_tariff_type == 'LV TOU <100MWh' or network_tariff_type == 'LV Business TOU_Interval meter' or network_tariff_type == 'Small Business - Opt in Demand':
            if 'TOU' in network_tariff_type:
                variable_tariff = get_tou_variable_tariff(time, my_tariffs.get_nuos_on_grid_import_variable(time,network_tariff_type))
                results.set_participant_nuos_payments(time, p.get_id(), variable_tariff * external_grid_import)
    ...
    for time in time_periods:
        nuos_grid_import_revenue_variable = sum([results.get_participant_nuos_payments(time, p.get_id()) for p in mynetwork.get_participants()])
    ...
    for time in time_periods:
        ...
        battery_import = sum([results.get_central_batt_solar_sales(time, p.get_id()) for p in mynetwork.get_participants()])
...
def get_tou_variable_tariff(time, tariffs):
    peak_charge, shoulder_charge, offpeak_charge, peak_start_time, peak_end_time, peak_start_time_2, peak_end_time_2, shoulder_start_time, shoulder_end_time, shoulder_start_time_2, shoulder_end_time_2, tou_weekday_only_flag, demand_charge = tariffs
    ...
    # In the case where TOU periods only apply on weekdays then check for weekdays.
    if tou_weekday_only_flag == 0 or (tou_weekday_only_flag == 1 and time.weekday() >= 0 and time.weekday() <=4):
        # Check whether it's shoulder time 1
        if (time.hour >= shoulder_start_time and time.hour <= shoulder_end_time) :
            variable_tariff = shoulder_charge
        # Check for whether it's a peak time. Actually only 1 peak time period for now.
        if (time.hour >= peak_start_time and time.hour <= peak_end_time) :
            variable_tariff = peak_charge
        # Check whether it's shoulder time 2
        if (time.hour >= shoulder_start_time_2 and time.hour <= shoulder_end_time_2) :
            variable_tariff = shoulder_charge
